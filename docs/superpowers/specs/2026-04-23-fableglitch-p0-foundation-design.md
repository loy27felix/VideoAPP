...
# GitHub
GITHUB_BOT_TOKEN=               # fableglitch-bot PAT, scopes: repo
...
1. **GitHub 组织 + bot 账号**
   - 创建或复用 `fableglitch` GitHub organization
   - 订阅 **GitHub Team** 计划（$4/seat/月，仅用 1 个 seat 给 bot）
   - 用 `fableglitch-bot@beva.com` 注册独立机器人账号，加入 org，生成 PAT（scopes: `repo`）
   - 创建私有仓库 `fableglitch/asset-library`，初始只含 `.gitignore` 和 README
   - 配你 + 最多 1 位 admin 为协作者；**其他 30 位创作者一律不加**
